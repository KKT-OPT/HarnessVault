...
status: draft
type: index
purpose: 索引 Harness Agent Capability Layer 的智能体能力、上下文、Prompt、Skill、Memory 和机器友好辅助入口。
...
## 3. Machine-checkable 辅助入口

| 文档 | 状态 | 用途 |
|---|---|---|
| `docs/agent/AgentContextManifest.yaml` | draft | 机器友好的辅助导航，定义 entry order、default include / exclude、layer index mapping 和 artifact boundary |

`AgentContextManifest.yaml` 不替代 Markdown 事实源。智能体仍应优先读取 `AGENTS.md`、[[INDEX]] 和 [[PLANS]]。
