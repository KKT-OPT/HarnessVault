...
## 2. Complex Task Prompt Rule

Complex Harness tasks should use `templates/ComplexTaskPromptTemplate.md` to separate entry contract, final goal, previous context, feedback, task, inspection scope, constraints, acceptance criteria, output format and governance notes.

Use this template when a task spans multiple documents, changes governance or index rules, carries prior conversation context, or has explicit acceptance criteria. The XML-like blocks are a prompt structure convention, not a mandatory schema.

Generic prompt templates must not contain real user knowledge or concrete project facts.
...
- Reusable source templates, including prompt templates, live under `templates/**`.
